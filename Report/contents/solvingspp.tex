...
\todo[inline]{not sure if the above paragraph makes sense\ldots}
...
\todo[inline]{draw a min-heap tree if have time \ldots?}
...
\todo[inline]{Andrea asks if it also depends on the network. - so how do I say it needs a `nice' network?}
...
\todo[inline]{our A* is kind of like preprocessing\ldots we need run Dijkstra on every node in the graph \ldots }
