...
\todo[inline]{The following sections will use big O, $f \in O(g) : f \leq g$, f = O(g): f does not grow asymptotically faster than g.}
...
\begin{algorithm}
    ...
    \If{$u \text{ is not zone } \text{\bfseries or } u = s$}
...
\todo[inline]{ref THE D'ESOPO-PAPE ALGORITHM, implementation and efficiency of moore-algorithms for the shortest route problem}
...
\todo[inline]{draw a min-heap tree?}
...
\todo[inline]{Say I will be comparing all these}
...
\todo[inline]{also depends on the network?}
...
\todo[inline]{nether the less, we implement and test this for traffic assignment}
...
\todo[inline]{How about data storage of trees? Check Michelle's report  on anything regarding under-estimation of time on trees}
...
\todo[inline]{make this clearer:one could possibly use 0-flows landmarks, but we didn't look into that further}

\section{Techniques for iterative calculations}
\todo[inline]{reformat, see page 23 and 24 in both fixes}
\todo[inline]{before talking about SP at beginning of chapter, should make it VERY clear that in each iteration a static SP is being solved, hence we use standard algos}
...
\todo[inline]{rewrite, see page 24 Olga fix}
...
\todo[inline]{rewrite, see page 24 Olga and Andrea fix}
