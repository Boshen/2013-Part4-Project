...
\todo[inline]{removed system optimal, the report does not mention system optimal anywhere anymore}
...
These assumptions may not be realistic \todo{keep this realistic part?} but they allow us to formulate the traffic assignment model as a mathematical program.
...
This means the equilibrium is the result of everybody simultaneously attempting to minimise their own travel times.
...
\todo[inline]{this section is borrowed from Olga's paper ``A Computational Study of Traffic Assignment Algorithms''}
...
As a result, the development of path-based algorithm has only began in the last decade, thus there are lots of improvements can be done.
...
In steps 3 and 4, traffic flows are redistributed between the longest and shortest paths, these steps are equivalent to solving the Wardrop equilibrium shown in Equation~\ref{wardrop1}-\ref{wardrop3}.
...
it first computes shortest path with fixed traffic flows and link costs for O-D pair $i$, moves the traffic flow to satisfy the Wardrop equilibrium,
...
The rate of convergence does not decrease monotonically. \todo{and \ldots}
