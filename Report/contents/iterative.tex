...
\todo[inline]{I also need better explaination on why avoiding sp still allow PE to converge.}
...
%\todo[inline]{new paragraph, I Just realised all SP calculations may be skipped \ldots, resulting PE to stop}
%Upon investigation,
%it is realised that this strategy has an non-converging edge case for the path equilibration algorithm.
%The edge case happens when all of the shortest path calculations are skipped in two consecutive iterations.
%For example if half of the shortest path calculations are skipped for the next five iterations in the second iteration,
%and the other half are skipped for the next five iterations in the third iteration,
%then accidentally all calculations are skipped in the fourth and fifth iteration,
%resulting the path equilibration algorithm to stop with reletative gap 
