...
ObjectManager::ObjectManager(Params *params, Timer &timer) : timer_(timer){
	_params = params;
...
bool ObjectManager::getSparsity(){
	if (_useSparsity == -1) {
		std::string tmp = _params->getParamWoSpaces("SPARSITY");
		//Utils::removeWhiteSpaces(tmp);
		if (tmp == "1") {
			_useSparsity = 1;
		} else if (tmp == "0") {
...
PathSet* ObjectManager::getPathSet(){
	if (_pathSet == NULL) {
		...
		if (app == APP3) {
			...
			if (algo == PE) {
				_pathSet = new PathSet(getNet(), getODMatrix(), getShPath(), getConvMeasure(), getPathCost(), getFloatValue("ZERO_FLOW"), getPathAlgoCreator());
			} else if (algo == GP) {
				_pathSet = new PathSet(getNet(), getODMatrix(), getShPath(), getConvMeasure(), getPathCost(), getFloatValue("ZERO_FLOW"), getFloatValue("ALPHA"), getPathAlgoCreator());
			} else {
			...
			//} else {
				_pathSet = new PathSet(getNet(), getODMatrix(), getShPath(), getConvMeasure(), getPathCost(), getDerivative(), getLineSearch(), getPathAlgoCreator());
			//}
		}
	}
	return _pathSet;
...
DecoratedEqAlgo* ObjectManager::getEqAlgo(){
	if (_algo == NULL) {
		...
		if ((tmp == "FW") || (tmp == "CFW") || (tmp == "BFW")) {
			...
			_algo = new FWAlgo(_linkFlows, getAddHook());
		} else if (getPathAlgoType() != Nothing){
			std::string tmp = _params->getParamWoSpaces("EQUILIBRATION");
			//Utils::removeWhiteSpaces(tmp);
			//std::cout << "EQ = " << tmp << std::endl;
			if (tmp == "EQI") {
				_algo = new PathBasedAlgo(getPathSet(), true, getAddHook());
			} else if (tmp == "EQII") {
				//_eqII = new EquilibrationII();
				_algo = new PathBasedAlgo(getPathSet(), false, getAddHook());
			} else {
