...
void PathSet::initialiseObject(StarNetwork *net, ODMatrix *mat, ShortestPath *shPath, ConvMeasure *conv, PathCost *pathCost){
	_mat = mat;
	_shPath = shPath;
	_conv = conv;
...
PathSet::PathSet(StarNetwork *net, ODMatrix *mat, ShortestPath *shPath, ConvMeasure *conv, PathCost *pathCost, Derivative *der, LineSearch *lineSearch, PathAlgoCreator *algoCreator) {
	initialiseObject(net, mat, shPath, conv, pathCost);
	...
	for (Origin *origin = _mat->beginOrigin(); origin != NULL; origin = _mat->getNextOrigin()) {
		for (PairOD *dest = origin->beginDest(); dest != NULL; dest = origin->getNextDest()) {
			...
			_odSetList[odIndex] = new ODSetWithStep(odIndex, dest->getIndex(), origin->getIndex(), _pathCost, net, shPath, conv, der, _linkFlows, lineSearch, algoCreator);
		}
	}
	
};

/*PathSet::PathSet(StarNetwork *net, ODMatrix *mat, ShortestPath *shPath, ConvMeasure *conv, PathCost *pathCost, Derivative *der, LineSearch *lineSearch, FPType sparsityPrecision, PathAlgoCreator *algoCreator) {
	initialiseObject(net, mat, shPath, conv, pathCost);
	...
	for (Origin *origin = _mat->beginOrigin(); origin != NULL; origin = _mat->getNextOrigin()) {
		for (PairOD *dest = origin->beginDest(); dest != NULL; dest = origin->getNextDest()) {
			odIndex = dest->getODIndex();
			_odSetList[odIndex] = new ODSetWithStep(odIndex, dest->getIndex(), origin->getIndex(), _pathCost, net, shPath, conv, der, _linkFlows, lineSearch, sparsityPrecision, algoCreator);
		}
	}
	
};*/

PathSet::PathSet(StarNetwork *net, ODMatrix *mat, ShortestPath *shPath, ConvMeasure *conv, PathCost *pathCost, FPType zeroFlow, PathAlgoCreator *algoCreator){
	initialiseObject(net, mat, shPath, conv, pathCost);
	...
	for (Origin *origin = _mat->beginOrigin(); origin != NULL; origin = _mat->getNextOrigin()) {
		for (PairOD *dest = origin->beginDest(); dest != NULL; dest = origin->getNextDest()) {
			odIndex = dest->getODIndex();
			_odSetList[odIndex] = new ODSet(odIndex, dest->getIndex(), origin->getIndex(), _pathCost, net, shPath, conv, _linkFlows, zeroFlow, algoCreator);
		}
	}
};

PathSet::PathSet(StarNetwork *net, ODMatrix *mat, ShortestPath *shPath, ConvMeasure *conv, PathCost *pathCost, FPType zeroFlow, FPType alpha, PathAlgoCreator *algoCreator){
	initialiseObject(net, mat, shPath, conv, pathCost);
	...
	for (Origin *origin = _mat->beginOrigin(); origin != NULL; origin = _mat->getNextOrigin()) {
		for (PairOD *dest = origin->beginDest(); dest != NULL; dest = origin->getNextDest()) {
			odIndex = dest->getODIndex();
			_odSetList[odIndex] = new ODSet(odIndex, dest->getIndex(), origin->getIndex(), _pathCost, net, shPath, conv, _linkFlows, zeroFlow, alpha, dest->getDemand(), algoCreator);
		}
	}
};
