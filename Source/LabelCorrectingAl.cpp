...
LabelCorrectingAl::LabelCorrectingAl(StarNetwork *netPointer, CounterContainer &container) : OperationCounter(container), _netPointer(netPointer), _nbNodes(netPointer->getNbNodes()), _originIndex(-1), nbCalls_(0) {
...
std::string LabelCorrectingAl::getCounterValue(){
	std::stringstream ss;
	ss << "<LC nbCalls> : {" << nbCalls_ << "}";
	return ss.str();
};
