...
class ShPathAstarSTL : public ShPathInterface {
    ...
    public:
        ShPathAstarSTL(StarNetwork* netPointer, ODMatrix* _odMatrix):ShPathInterface(netPointer){
            ...
            _nDests = 1+*destinations.rbegin();

            zeroFlowTimes = new std::vector<FPType>(_nNodes*_nDests); 
            ...
            for(int i = 0; i < _nNodes; i++){
                ...
                for (std::set<int>::iterator it=destinations.begin(); it!=destinations.end(); ++it){
                    zeroFlowTimes->at(i*_nDests+*it) = LabelVector->at(*it);
                }
            ...
            while ( !Q.empty() ){
                ...
                for (nextLink = NP.beginLink();
                    ...
                    if ( Duv < L[v] ){
                        L[v] = Duv;
                        P[v] = nextLink->getIndex();
                        Q.push(ShPath::PQPair(Duv + H[v*_nDests+D], v));
                    }
