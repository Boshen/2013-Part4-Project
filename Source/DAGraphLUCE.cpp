...
DAGraphLUCE::DAGraphLUCE(StarNetwork *net, ODMatrix *mat, FPType zeroFlow, FPType dirTol, int originIndex) : DAGraph(net, mat, zeroFlow, originIndex),
...
void DAGraphLUCE::computeAvCosts(std::vector<FPType> &C, std::vector<FPType> &G){
  ...
  for (; i != -1; i = getNextAscPass()){
	...
	C[i] = minVal;
	if (sum2 == 0.0) {
	  std::cout << "We have zero derivative here G[i] = 0, i = " << i << std::endl;
	...
	while (lambda) {
	  ...
	  for (std::list<StarLink*>::const_iterator it = inLinks.begin(); it != inLinks.end(); ++it){
	    ...
	    if (fabs(tmp) <= getZeroFlow()){
	      ...
	      tmp = 1.0;
	      // std::cout << "We have zero derivative here tmp= 0, linkIndex = " << linkIndex  << " nodeFromIndex = " << nodeFromIndex << " origin = " << getOriginIndex() << std::endl;
	    }
