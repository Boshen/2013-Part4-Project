...
class ShPathAstar : public ShPathInterface {
    ...
    public:
        ShPathAstar<PriorityQueue>(StarNetwork* netPointer):ShPathInterface(netPointer){
            ...
            for(int i = 0; i < _nNodes; i++){ // using D as number of zone nodes
                calculate(i);
                    ...
                    for (StarLink *nextLink = _netPointer->beginLink(); nextLink != NULL; nextLink = _netPointer->getNextLink()) {
                        ...
                        if ( Duv < N[v].dist ){
                            ...
                            if( L[v] == UNREACHED ){
                                ...
                                //if(O == 0 && D == 10)
                                //    std::cout << "push " << v << " | Duv " << Duv << " | Fuv " << Fuv << " | HvD " << H[v*_nNodes+D] << std::endl;
                            }else if( L[v] == SCANNED){
                                Q.increase(K[v], ValueKey(v, -Fuv));
                                //if(O == 0 && D == 10)
                                //    std::cout << "incr " << v << " | Duv " << Duv << " | Fuv " << Fuv << " | HvD " << H[v*_nNodes+D] << std::endl;
                            }
                        }
                    } // for each outgoing link
                    //            if(O == 0 && D == 10)
